...
Система спроектирована как **простой внутренний инструмент без входа с
паролем** (кроме `/admin/`). Это осознанное решение для работы на одном
компьютере. Как только вы разворачиваете её на сервере и открываете доступ
по сети — **любой, кто может достучаться до сервера, сможет редактировать
и удалять данные** без какой-либо аутентификации.
...
- При необходимости ограничьте доступ на уровне сетевого экрана/файрвола —
  только с IP-адресов отдела АСУТП.
- Если понадобится реальный вход с паролем для всех пользователей (не
  только для `/admin/`) — это отдельная доработка (Django `login_required`
  на всех views), напишите отдельным запросом, если нужно.
...
`http://<адрес-сервера>:8000/` — система должна отвечать.
...
2. Дайте сотрудникам ссылку на `http://<адрес-сервера>/` — это и есть
   рабочий интерфейс системы.
3. Настройте регулярный бэкап (см. ниже) — на сервере это особенно важно.
